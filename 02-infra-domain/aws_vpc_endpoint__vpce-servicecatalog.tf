...
# aws_vpc_endpoint.vpce-0adee5bcfff83a9c0:
resource "aws_vpc_endpoint" "vpce-servicecatalog" {
  private_dns_enabled = true
  route_table_ids     = []
  security_group_ids = [
    aws_security_group.sg-0b49e7f1e576d908d.id,
  ]
  service_name = format("com.amazonaws.%s.servicecatalog", data.aws_region.current.name)
  subnet_ids = [
